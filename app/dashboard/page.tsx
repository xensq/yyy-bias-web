...
const TABS = ["bias", "topology", "gex", "net iv", "macro"] as const
        ...
        <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 24px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: "48px" }}>
              ...
              <nav style={{ display: "flex", gap: "4px" }}>
                {TABS.map(t => (
                  <button key={t} onClick={() => setTab(t)}
                    ...
                    onMouseLeave={e => { if (tab !== t) e.currentTarget.style.color = "#3a3a3a" }}
                  >
                    {t}
                  </button>
                ))}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
              <p style={{ fontSize: "11px", color: "#333" }}>{now}</p>
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "24px" }}>
        {loading && (
          <div style={{ display: "flex", alignItems: "center", gap: "10px", color: "#444", fontSize: "12px", marginTop: "48px" }}>
            <div style={{ width: "6px", height: "6px", background: "var(--accent)", borderRadius: "50%", animation: "pulse 1.5s infinite" }} />
            pulling live data...
          </div>
        )}
        {error && <div style={{ border: "0.5px solid rgba(255,85,85,0.3)", background: "rgba(255,85,85,0.05)", borderRadius: "8px", padding: "16px", color: "#ff5555", fontSize: "12px", marginTop: "16px" }}>{error}</div>}
        {data && !loading && (
          <>
